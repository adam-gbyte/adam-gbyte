...
>
  ...
  <!-- Mobile Menu Button -->
  <button class="md:hidden mt-4" on:click={toggleMenu} aria-label="Toggle Menu">
    {#if isMenuOpen}
      <X />
    {:else}
      <Menu />
...
<aside
  class={`absolute top-0 left-0 h-full w-64 bg-gray-200 shadow-lg z-60 border-r border-gray-300 transform transition-transform duration-500 ease-in-out p-6 ${
    isMenuOpen ? "translate-x-0" : "-translate-x-full"
  }`}
>
  <h2 class="text-lg font-bold mb-4">Sidebar</h2>
  <p class="text-gray-700">
    Ini adalah sidebar dengan animasi menggunakan TailwindCSS.
  </p>
</aside>
