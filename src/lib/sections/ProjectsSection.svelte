...
<section
	id="projects"
	class="relative mx-auto flex w-full max-w-7xl flex-col gap-16 overflow-hidden px-4 py-24 sm:px-6 lg:py-32"
>
	<!-- Background Glow -->
	<div
		class="pointer-events-none absolute top-0 left-1/2 -z-10 h-[500px] w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-emerald-500/10 blur-[100px]"
	></div>
	...
	<div class="flex w-full flex-col gap-8 lg:flex-row">
		...
		>
            ...
            {i === active
						? 'scale-[1.02] bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/20'
			...
			{#each tabs as tab, i}
				{#if i === active}
					<div
						...
						class="group relative overflow-hidden rounded-[2rem] border border-white/40 bg-white/60 p-6 shadow-2xl backdrop-blur-2xl sm:p-10 dark:border-white/10 dark:bg-slate-900/60"
					>
						<!-- Decorative gradient behind panel -->
						<div
							class="absolute -top-20 -right-20 -z-10 h-[300px] w-[300px] rounded-full bg-emerald-500/10 blur-[80px]"
						></div>
						...
						<div class="flex flex-col gap-10 xl:flex-row">
							...
							<div class="flex flex-col gap-6 xl:w-5/12">
								<div>
									...
									{#if tab.subtitle}
										<div
											class="mt-2 flex items-center gap-2 text-sm font-medium text-emerald-600 dark:text-emerald-400"
										>
											<Zap class="size-4" />
											{tab.subtitle}
										</div>
								...
								{#if tab.tags}
									<div class="flex flex-wrap gap-2">
										{#each tab.tags as t}
											<span
												class="rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600 dark:border-white/10 dark:bg-slate-800/50 dark:text-slate-300"
											>
												{t}
											</span>
										{/each}
							...
							<div class="relative xl:w-7/12">
								<div
									class="group/slider relative aspect-[16/10] overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-xl dark:border-white/10 dark:bg-slate-800"
								>
									<div
									...
									>
										{#each tab.images as image, idx}
											<div class="relative h-full w-full flex-shrink-0">
												...
												<div
													class="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-black/50 to-transparent opacity-60"
												></div>
											</div>
										{/each}
									</div>
                          ...
                          {i === current ? 'w-6 bg-white' : 'w-1.5 bg-white/50 hover:bg-white/80'}"
									...
									<button
										...
										class="absolute top-1/2 left-4 -translate-y-1/2 rounded-full bg-white/10 p-2 text-white/90 opacity-0 backdrop-blur-md transition group-hover/slider:opacity-100 hover:scale-110 hover:bg-white/20 dark:bg-black/30"
									>
									...
									<button
										...
										class="absolute top-1/2 right-4 -translate-y-1/2 rounded-full bg-white/10 p-2 text-white/90 opacity-0 backdrop-blur-md transition group-hover/slider:opacity-100 hover:scale-110 hover:bg-white/20 dark:bg-black/30"
									>
