...
>
	<!-- Main Card -->
	<div
		class="relative flex w-full max-w-2xl flex-col items-center gap-8 rounded-[2.5rem] border border-white/40 bg-white/60 p-8 text-center shadow-2xl backdrop-blur-2xl transition-all duration-500 hover:shadow-emerald-500/10 sm:p-14 dark:border-white/10 dark:bg-slate-900/60"
	>
		<!-- Grid Pattern Overlay (Optional Texture) -->
		<div
			class="absolute inset-0 -z-10 bg-[url('/grid.svg')] [mask-image:linear-gradient(180deg,white,rgba(255,255,255,0))] bg-center opacity-50"
		></div>
		...
		<div class="flex flex-col items-center gap-4">
			...
			<p class="max-w-lg text-base leading-relaxed text-slate-600 sm:text-lg dark:text-slate-300">
				Have a project in mind, a question, or just want to collaborate? I’m always open to
		...
		<!-- Note -->
		<div
			class="mt-2 flex items-center gap-2 text-xs font-medium text-slate-500 sm:text-sm dark:text-slate-400"
		>
	...
	></div>
	<div
		class="pointer-events-none absolute top-0 right-1/4 -z-10 h-[300px] w-[300px] translate-x-1/2 -translate-y-1/2 rounded-full bg-teal-500/10 blur-[80px]"
	></div>
</section>
